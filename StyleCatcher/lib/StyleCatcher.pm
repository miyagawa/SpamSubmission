...
sub view {
    ...
    if ($blog_id && @$styled_blogs) {
        ...
        $app->add_breadcrumb(MT->translate("Main Menu"), $app->mt_uri);
        $app->add_breadcrumb($blog->name, $app->mt_uri(mode => 'menu', args => { blog_id => $blog_id }));
        $app->add_breadcrumb(MT->translate('Templates'), $app->mt_uri(mode => 'list', args => { _type => 'template', blog_id => $blog_id }));
    } else {
        $app->add_breadcrumb(MT->translate("Main Menu"), $app->mt_uri);
        $app->add_breadcrumb(MT->translate("System Overview"), $app->mt_uri(mode => 'admin'));
        $app->add_breadcrumb(MT->translate("Plugins"), $app->mt_uri(mode => 'list_plugins'));
    }
...
sub apply {
    ...
    if ($url !~ m/^\Q$sys_webthemeroot\E/) {
        ...
        my @images = $response->content =~ m/(?<=url\().+(?=\))/g;
        $filemgr->mkpath(File::Spec->catdir($sys_themeroot, $basename))
            or return $app->json_error("Could not create $basename folder - Check that your 'themes' folder is webserver-writable.");
    ...
    unless ($replaced) {   
        ...
        @template_lines = (<<EOT);
$header
\@import url($webthemeroot/base-weblog.css);
\@import url($url);
...
EOT
    ...
    $app->send_http_header('text/plain');
    $app->{no_print_body} = 1;
    $app->print("Successfully applied new theme selection.");
}
...
sub make_themes {
    ...
    # if we have a themes structure in our session, then preload the
    # catcher with that data. otherwise, leave it empty to signal to the
    # catcher that it needs to invoke a request for it and load it "in the
    # background".
